...
// base query which automatically reauthenticates the user if his/her access token expires (similarly to axios interceptors)
  ...
  if (
    ...
    if (!mutex.isLocked()) {
      ...
      try {
        ...
        // @ts-ignore
        const newAccessToken: string = refreshResultData?.newAccessToken;
        // @ts-ignore
        const user: IUser = refreshResultData?.user;

        if (newAccessToken && user) {
          // dispatch the 'setCredentials' action (set the user and the new access token in global state)
          api.dispatch(
